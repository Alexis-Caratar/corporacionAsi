...
$datos = Usuario::getDatosEnObjetos("identificacion", null);

if (count($datos) > 0) {
    for ($i = 0; $i < count($datos); $i++) {
        $usuario = $datos[$i];
        $lista .= '<tr class="gradeC">';
        $lista .= '<td class="text-center">' . strtoupper($usuario->getIdentificacion()) . '</td>';
        $lista .= '<td class="text-center">' . strtoupper($usuario->getNombres()) . '</td>';
        $lista .= '<td class="text-center">' . strtoupper($usuario->getApellidos()) . '</td>';
        $lista .= '<td class="text-center">' . strtoupper($usuario->getDireccion()) . '</td>';
        $lista .= '<td class="text-center">' . strtoupper($usuario->getTelefono()) . '</td>';
        $lista .= '<td class="text-center">' . strtoupper($usuario->getRolFin()->getRol()) . '</td>';
        ...
        $lista .= "<a href='#' onClick='permisosCU(" . $menu1 . ",{$user->getIdentificacion()}," . '"' . "principal2.php?CONTENIDO=admon/usuario/usuariosFinalFormulario.php&accion=Modificar&identificacion={$usuario->getIdentificacion()}" . '"' . "," . '"' . "U" . '"' . ")' class='dropdown-item edit' data-toggle='tooltip' data-placement='bottom' title='Modificar'><img src='assets/icon/modificar.png' width='15px' alt=''  /></a>";
        $lista .= "<a ' href='principal2.php?CONTENIDO=admon/OpcionAcceso/opcionacceso.php&identificacion={$usuario->getIdentificacion()}&n_user={$usuario->getNombresCompletos()}&menu1={$menu1}'> <img src='assets/icon/opcion.png' width='20px' alt='' class='dropdown-item' data-toggle='tooltip' data-placement='bottom' title='permisos' /></a>";
        $lista .= "<a href='#' onClick='eliminar({$usuario->getIdentificacion()}," . $menu1 . ",{$user->getIdentificacion()}," . '"' . "D" . '"' . ")' class='dropdown-item' data-toggle='tooltip' data-placement='bottom' title='Eliminar'> <img src='assets/icon/eliminar.png' width='20px' alt=''  /></a>";
        $lista .= "</div>";
        $lista .= "</div>";
        $lista .= "</div>";
        $lista .= '</td>';
        ...
        <div class="row">
            <div class="col-xs-13 lead">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="#" data-toggle='tooltip' data-placement='bottom' title='Adicionar' onclick="permisosCU(<?=$menu1?>,<?=$user->getIdentificacion()?>,'principal2.php?CONTENIDO=admon/usuario/usuariosFinalFormulario.php&accion=Adicionar&','C')"    >Nuevo usuario</a></li>
                </ul>
            </div>
        </div>
    </div>
...
//funcion de validacion del permiso
                    function eliminar(identificacion,idmenu,idusuario,accion) {
        var request = $.ajax({
                    type: "POST",
                ...
                request.done(function (transporte) {
                    ...
                    if (transport['flag'] === false) {
                        ...
                        location = 'principal2.php?CONTENIDO=admon/usuario/usuariosFinalActualizar.php&accion=Eliminar&identificacion=' +identificacion + '&menu1=<?=$menu1?>';
                    }
                });
